...
@Form.create() 
export default class SurDetail extends PureComponent {
  static contextTypes = {
  ...
  handleDateRangeChange = date => {
    this.mergeSearchValues({
      debitStartDate: date[0].format('YYYY-MM-DD'),
      debitEndDate: date[1].format('YYYY-MM-DD')
    })
  }
  ...
  render() {
    const { global: {floors, areas}, adv: {surData:{records, current, total}}, loading, floorFetch, areaFetch } = this.props;
    ...
    const columns = [
      ...
      },
      {
        title: '扣款金额',
        dataIndex: 'debit',
        render: value => <span style={{color: value <0 ? 'red' : 'green'}}>{value}元</span>
      },
      {
        title: '扣款日期',
        dataIndex: 'debitDate',
